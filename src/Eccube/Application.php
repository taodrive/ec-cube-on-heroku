...
class Application extends ApplicationTrait
{
    ...
    public function parseConfig($config_name, array &$configAll, $wrap_key = false, $ymlPath = null, $distPath = null)
    {
        ...
        //---------------------------------------------------------------------------
        // 環境変数:ENVの値によって、読み込むconfigファイルを切り替える
        //---------------------------------------------------------------------------
        $env = getenv('ENV');
        $env = $env ? $env : '';
        if (!!strcmp($env, '')) {
            // 読み込む対象が 'database' の場合
            if (!strcmp($config_name, 'database')) {
                $config_php = $ymlPath.'/'.$env.'/'.$config_name.'.php';
            }
            // 読み込む対象が 'session_handler' の場合
            if (!strcmp($config_name, 'session_handler')) {
                $config_php = $ymlPath.'/'.$env.'/'.$config_name.'.php';
            }
        }
        ...
        //---------------------------------------------------------------------------
        // 環境変数:ENVの値によって、定義されている差分のconfigファイルを読み込んで上書きする
        //---------------------------------------------------------------------------
        if (!!strcmp($env, '')) {
            // pathの設定を上書きする
            if (!strcmp($config_name, 'path')) {
                $override_config_php = $ymlPath.'/'.$env.'/'.$config_name.'.php';
                if (file_exists($override_config_php)) {
                    $override_config = require $override_config_php;
                    $config = array_merge($config, $override_config);
                }
            }
            // configの設定を上書きする
            if (!strcmp($config_name, 'config')) {
                $override_config_php = $ymlPath.'/'.$env.'/'.$config_name.'.php';
                if (file_exists($override_config_php)) {
                    $override_config = require $override_config_php;
                    $config = array_merge($config, $override_config);
